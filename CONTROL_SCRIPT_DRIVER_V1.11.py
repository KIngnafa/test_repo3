...
if comlin_args == 0:
	...
	if function == "database_backup":
		...
		if email_send == "Yes":
			...
			status=k.database_backup(schema,runner,directory,par_dir,sourcedb_physical,dbname)
	...
	if comlin_args == 7:
		...
		#calling database backup fucntion with required arguments
		status=k.database_backup(schema,runner,directory,par_dir,sourcedb_physical,dbname)
		k.STACK_EMAIL(email_add,status,sys.argv[1],runner)
	...
	if comlin_args == 2:
		...
		#calling unzip function with required argument
		status=k.G_zipp(file_dir)
		k.STACK_EMAIL(email_add,status,sys.arv[1],runner)
	...
	elif comlin_args != 9 and comlin_args != 10:
		print()
		print("Incorrect arguments")
