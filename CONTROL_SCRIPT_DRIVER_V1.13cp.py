...
if comlin_args == 0:
	...
	if function == "database_backup":
		...
		if email_send == "yes":
			...
			status=k.data_migration(schema=schema,runner=runner,directory=directory,sourcedb_physical=sourcedb_physical,par_dir=par_dir,importdb_physical=importdb_physical,source_dbname=source_dbname,target_dbname=target_dbname,import_schema=import_schema,dmp_file=dmp_file)
			#calling stack email function with required arguments
			k.STACK_EMAIL(email_add=email_add,status=status,function=function,runner=runner)
		elif email_send == "no":
			#calling data_migration function
			k.data_migration(schema=schema,runner=runner,directory=directory,sourcedb_physical=sourcedb_physical,par_dir=par_dir,importdb_physical=importdb_physical,source_dbname=source_dbname,target_dbname=target_dbname,import_schema=import_schema,dmp_file=dmp_file)	
		else:
			print("Incorrect Option Entered!")
			print("Please Enter Yes or No!")
