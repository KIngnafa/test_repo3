...
def disk_utilization_check(**args):
   #output of subprocess.getoutput is stored in disk_util variable and type casted to an Integer
	...
	try:
		if disk_util > args["threshold"]:
			print("disk utilization for %s is above %s!"%(args["disk"],args["threshold"]))
		else:
			print("disk utilization for %s is below %s!"%(args["disk"],args["threshold"]))
		status="success"
	except:
...
def data_migration(**args):
	dblog_path="%sdb_status.log"%(args["par_dir"])
	import_log="%s_%s_%s.log"%(args["schema"],args["import_schema"],TS)
	importlog_path=os.path.join(args["importdb_physical"],import_log)
	...
	#calling unzipp function to unzip gzipped dmp file and extract it in the target db backup location
	unzipp(file_dir=sourcedmp_path,destination=args["importdb_physical"])
	...
	try:
		...
		par.write("userid='/ as sysdba'\n")
		par.write("schemas=%s\n"%(args["schema"]))
		par.write("remap_schema=%s:%s_%s_migrated\n"%(args["schema"],args["schema"],args["import_schema"]))
		par.write("dumpfile=%s_%s_%s.dmp\n"%(args["schema"],args["runner"],TS))
		par.write("logfile=%s_%s_%s.log\n"%(args["schema"],args["import_schema"],TS))
		par.write("directory=%s\n"%(args["directory"]))
		par.write("table_exists_action=replace")
	...
	try:
		#creating.sh file for import
		import_exec=open("%simport_exec.sh"%(args["par_dir"]), "w+")
		import_exec.write(". /home/oracle/scripts/oracle_env_%s.sh\nimpdp parfile=impdp_%s_%s_%s.par"%(args["target_dbname"],args["schema"],args["import_schema"],TS))
		import_exec.close()
		import_content=("%simport_exec.sh"%(args["par_dir"]))
		os.popen("chmod 700 %s"%(import_content))
	except Exception as e:
